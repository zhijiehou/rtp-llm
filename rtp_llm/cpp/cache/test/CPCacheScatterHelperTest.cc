...
protected:
    ...
    createMlaCacheManager(int layer_num, int block_num, int block_size, size_t scale_bytes_per_token = 0) {
        ...
        auto mgr = std::make_shared<KVCacheManager>(config);
...
TEST_F(CPCacheScatterHelperTest, PrepareStagingPlanAllocatesCorrectBlocks) {
    ...
    CPCacheScatterHelper helper(mgr.get());
...
TEST_F(CPCacheScatterHelperTest, ScatterAndReleaseProducesContiguousLayout) {
    ...
    for (int v = 0; v < vblock_count; ++v) {
        for (int p = 0; p < cp_size; ++p) {
            ...
            cudaMemcpy(parts[0].addr, block_data.data(), block_bytes, cudaMemcpyHostToDevice);
        }
    }
    ...
    block_ids_holder->assign(decode_block_ids);
    ...
    for (int t = 0; t < total_tokens; ++t) {
        ...
        cudaMemcpy(block_data.data(), parts[0].addr, block_bytes, cudaMemcpyDeviceToHost);
        cudaDeviceSynchronize();
...
TEST_F(CPCacheScatterHelperTest, ScatterAndReleaseHandlesPartialLastBlock) {
    ...
    cudaMemcpy(decode_parts[0].addr, init_data.data(), block_bytes, cudaMemcpyHostToDevice);
    cudaDeviceSynchronize();

    helper.scatterAndRelease(std::move(plan), block_ids_by_group, mgr->cacheConfig(), layer_num, total_tokens);

    std::vector<uint8_t> result(block_bytes);
    cudaMemcpy(result.data(), decode_parts[0].addr, block_bytes, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
...
TEST_F(CPCacheScatterHelperTest, ScatterAndReleaseAlsoScattersScaleForPartialLastBlock) {
    ...
    for (int p = 0; p < cp_size; ++p) {
        ...
        cudaMemcpy(parts[0].addr, kv_block_data.data(), kv_block_bytes, cudaMemcpyHostToDevice);
        cudaMemcpy(parts[1].addr, scale_block_data.data(), scale_block_bytes, cudaMemcpyHostToDevice);
    }
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(decode_parts[0].addr, init_kv.data(), kv_block_bytes, cudaMemcpyHostToDevice);
    cudaMemcpy(decode_parts[1].addr, init_scale.data(), scale_block_bytes, cudaMemcpyHostToDevice);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(kv_result.data(), decode_parts[0].addr, kv_block_bytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(scale_result.data(), decode_parts[1].addr, scale_block_bytes, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
...
TEST_F(CPCacheScatterHelperTest, ScatterPackedIndexerScaleLayout) {
    ...
    for (int p = 0; p < cp_size; ++p) {
        ...
        cudaMemcpy(parts[1].addr, packed_scale.data(), scale_block_bytes, cudaMemcpyHostToDevice);
    }
    cudaDeviceSynchronize();

...
torch::Tensor genBaseCache(const int   rope_dim,
    ...
    // Compute entirely on GPU to avoid CPU multi-threading non-determinism
    ...
    auto inv_freq =
        ...
        / torch::pow(
            rope_theta,
            torch::arange(0, rope_dim, 2, torch::TensorOptions(torch::kInt64).device(torch::kCUDA)).to(torch::kFloat32)
                / rope_dim);
    auto t =
        torch::arange(max_position_embeddings * rope_scale, torch::TensorOptions(torch::kInt64).device(torch::kCUDA))
            .to(torch::kFloat32);
    t.div_(rope_scale);
...
torch::Tensor genYarnCache(const int   rope_dim,
                           ...
                           const bool  interleave) {
    // Compute entirely on GPU to avoid CPU multi-threading non-determinism
    auto  cuda_opts = torch::TensorOptions(torch::kInt64).device(torch::kCUDA);
    auto  pos_freqs = torch::pow(rope_theta, torch::arange(0, rope_dim, 2, cuda_opts).to(torch::kFloat32) / rope_dim);
    ...
    auto linear        = (torch::arange(rope_dim / 2, cuda_opts).to(torch::kFloat32) - low) / (high - low);
    ...
    auto t             = torch::arange(max_position_embeddings * rope_scale, cuda_opts).to(torch::kFloat32);
    auto freqs         = torch::outer(t, inv_freq);

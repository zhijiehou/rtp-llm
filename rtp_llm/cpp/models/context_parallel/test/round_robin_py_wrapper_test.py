...
class TestRoundRobinPlan(unittest.TestCase):
    ...
    def test_with_padding(self):
        ...
        total_tokens = self.fake_input_tokens[
            :6
        ]  # 6 tokens, need 8 (cp_size=2, align to 2)
        cp_size = 2
        cp_chunk_size = 4
    ...
    def test_multiple_ranks_cp4(self):
        ...
        expected_tokens = [[total_tokens[i] for i in idx] for idx in expected_indices]
        ...
        for cp_rank in range(cp_size):
            ...
            self.assertEqual(
                indices, expected_indices[cp_rank], f"rank {cp_rank} indices mismatch"
            )
            self.assertEqual(
                tokens, expected_tokens[cp_rank], f"rank {cp_rank} tokens mismatch"
            )
...
class TestRoundRobinRestoreIndices(unittest.TestCase):
    ...
    def test_multi_stream(self):
        ...
        expected = torch.tensor(
            [0, 6, 1, 7, 2, 8, 3, 9, 4, 10, 5, 11], dtype=torch.int32
        )
        self.assertTrue(
            torch.equal(restore_indices, expected),
            f"Expected {expected.tolist()}, got {restore_indices.tolist()}",

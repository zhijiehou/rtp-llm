...
def merge_qkv(ts: List[torch.Tensor]) -> torch.Tensor:
    """Merge separate q, k, v weight tensors into [q; k; v]."""
...
class KdaLinearAttnAtomicWeight(LinearAttnAtomicWeight):
    """LinearAttnAtomicWeight with KDA-specific split for dt_bias.

    KDA dt_bias is [num_heads * head_dim] (per-dim vector),
    unlike GDN dt_bias which is [num_heads] (scalar per head).
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.split_func_factory = dict(_linear_attn_split_stratey)
        self.split_func_factory[W.linear_attn_dt_b] = split_kda_dt_bias

...
class RoundRobinSparseMlaFp8CPOpTest(TestCase):
    ...
    def _build_params_diff_block_size(
        ...
        # block_table uses physical_block_size (= virtual_block_size / cp_size)
        max_seq_len = max(seq_lengths)
        ...
        # fill_params uses physical_block_size
        mla_params = rtp_llm_ops.SparseMlaParams()
        mla_params.fill_params(
            attn_inputs,
            physical_block_size,
        ...
        # kv_cache uses physical_block_size for the page dimension
        kv_cache_base = (
            (
                torch.randn(
                    total_blocks,
    ...
    def test_diff_block_size_virtual_block_size(self):
        ...
        params = self._build_params_diff_block_size(
            [16], [0], physical_block_size=64, kernel_block_size=256
        )
    ...
    def test_diff_block_size_pack_src_slots(self):
        ...
        # physical cache has total_blocks * physical_bs slots
        total_physical_slots = params["total_blocks"] * physical_bs
        self.assertTrue(
            (src_slots >= 0).all(),

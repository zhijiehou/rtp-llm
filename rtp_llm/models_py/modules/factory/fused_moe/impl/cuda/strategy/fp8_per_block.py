...
class CudaFp8PerBlockEpElasticContiguousStrategy(MoeStrategy):
    """CUDA FP8 PerBlock EP elastic Contiguous strategy.

    Selected when ``USE_DEEPEP_ELASTIC=1`` with ``DEEPEP_ELASTIC_DO_EXPAND=1`` —
...
class CudaFp8PerBlockEpElasticHybridStrategy(MoeStrategy):
    """CUDA FP8 PerBlock EP elastic Hybrid strategy.

    Selected when ``USE_DEEPEP_ELASTIC=1`` with ``DEEPEP_ELASTIC_DO_EXPAND=0`` —
    pairs the elastic router with ``DeepGemmHybridExecutor``,
    using the same ep_scatter/ep_gather path as the normal DeepEP strategy.
    """
    ...
    def check_conditions(cls, checker: Any, config: MoEConfigAdapter) -> None:
        ...
        checker.check(not do_expand)
        checker.check(do_cpu_sync)
        checker.check(
            config.moe_strategy == "fp8_per_block_ep_elastic_hybrid"
            or config.moe_strategy == "auto"
        )
...
class CudaFp8PerBlockEpElasticDecodeStrategy(MoeStrategy):
    ...
    the elastic router decode-mode layout with ``DeepGemmMaskedExecutorV2``.
    """

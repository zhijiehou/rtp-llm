...
class DeepGemmContiguousExecutor(FusedMoeExpertExecutor):
    ...
    def execute(
        ...
        # expert_num_tokens 已由 router 传入，是 per-expert 的 token 数（GPU tensor）
        expert_num_tokens = payload.expert_tokens_meta.expert_num_tokens
        assert expert_num_tokens is not None, (
            "DeepGemmContiguousExecutor requires expert_num_tokens in expert_tokens_meta."
        )
        all_tokens = hidden_states_fp8.shape[0]
        num_experts_local = expert_num_tokens.size(0)
        ...
        # 构建 m_indices：数据已由 DeepEP dispatch (do_expand=True) 按专家连续排列，
        # 无需 ep_scatter，只需生成 per-row 索引告诉 grouped GEMM 每行属于哪个专家
        m_indices = torch.repeat_interleave(
            torch.arange(num_experts_local, device=hidden_states_fp8_device, dtype=torch.int32),
            expert_num_tokens.to(torch.int64),
        )
        ...
        if topk_weights is not None:
            # topk_weights 在 expand 模式下是 [num_expanded_tokens] 1D tensor，
            # 需要 unsqueeze(1) 才能和 [num_expanded_tokens, K] 的 down_output 正确广播
            down_output *= topk_weights.unsqueeze(1)

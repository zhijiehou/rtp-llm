...
# ============================================================================
# Tests: Gate kernel vs naive
# ============================================================================


def test_gate_vs_naive():
    """Compare fused_kda_gate kernel output against naive PyTorch gate."""
    _, _, _, g_raw, _, A_log, dt_bias, _ = make_kda_inputs(1, 8)
    # Kernel: expects [tokens, H, K] reshaped input
    ...
    assert sim > COS_THRESH, f"Gate vs naive cos_sim={sim:.6f}"
    assert (gate_kernel <= 1e-6).all(), "Gate values should be non-positive"
    logging.info(f"test_gate_vs_naive PASSED (cos_sim={sim:.6f})")


def test_gate_no_bias_vs_naive():
    ...
    sim = cos_sim(gate_kernel, gate_naive)
    assert sim > COS_THRESH, f"Gate no-bias vs naive cos_sim={sim:.6f}"
    logging.info(f"test_gate_no_bias_vs_naive PASSED (cos_sim={sim:.6f})")


# ============================================================================
# Tests: Decode kernel vs naive (inplace_final_state=True, T=1 per real decode)
# ============================================================================


def test_decode_vs_naive(B):
    """Compare fused_recurrent_kda (inplace_final_state=True) against naive recurrent.
    Decode always uses T=1 in production. Clone state before passing to kernel.
    """
    ...
    # inplace_final_state=True: fs_kernel is state_for_kernel (mutated in-place)
    ...
    assert sim_o > COS_THRESH, f"Decode output cos_sim={sim_o:.6f} (B={B})"
    assert sim_s > COS_THRESH, f"Decode state cos_sim={sim_s:.6f} (B={B})"
    logging.info(f"test_decode_vs_naive PASSED B={B} (o={sim_o:.6f}, s={sim_s:.6f})")


# ============================================================================
# Tests: Prefill kernel vs naive (verify both output and final_state)
# ============================================================================


def test_prefill_vs_naive(B, seq_len):
    """Compare chunk_kda prefill against naive recurrent (output + final_state)."""
    q, k, v, g_raw, beta, A_log, dt_bias, state = make_kda_inputs(B, seq_len)
    scale = K**-0.5
    ...
    assert sim_s > COS_THRESH, f"Prefill state cos_sim={sim_s:.6f} (B={B}, T={seq_len})"
    logging.info(
...
# ============================================================================
# Tests: Prefill-decode consistency
# ============================================================================


def test_prefill_then_decode():
    """Prefill 11 tokens, then decode 1 token. Compare with prefill of 12 tokens."""
    q_all, k_all, v_all, g_all, beta_all, A_log, dt_bias, state = make_kda_inputs(1, 12)
    ...
    assert sim > COS_THRESH, f"Prefill-decode consistency failed: cos_sim={sim:.6f}"
    logging.info(f"test_prefill_then_decode PASSED (cos_sim={sim:.6f})")


if __name__ == "__main__":
    logging.info("=== Gate vs Naive ===")
    test_gate_vs_naive()
    test_gate_no_bias_vs_naive()

    logging.info("=== Decode vs Naive (inplace_final_state=True, T=1) ===")
    for bs in [1, 4, 8]:
        test_decode_vs_naive(bs)

    logging.info("=== Prefill vs Naive (output + state) ===")
    for bs in [1, 2]:
        for seq in [11, 64, 128]:
            test_prefill_vs_naive(bs, seq)

    logging.info("=== Prefill-Decode Consistency ===")
    test_prefill_then_decode()

    logging.info("All KDA tests passed.")
